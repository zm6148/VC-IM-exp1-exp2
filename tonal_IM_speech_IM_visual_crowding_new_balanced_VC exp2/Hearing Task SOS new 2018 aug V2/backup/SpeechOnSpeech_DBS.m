...
%------------------------------------------------------------------
raw_stim_directory = 'E:\Google Drive\HumanProjects\Pelli\SpeechOnSpeechMaskingIM\Thuy\RawSounds\BU Edited Words'; %stimulus directory
    ...
    for n_trial = Params.current_trial : Params.TrialsPerBlock
        ...
        for n_col = 1:5
            ...
            for which_band = Params.band_list_target(n_block,n_trial,:)
                ...
                %analysis filter
                [ban,aan] = butter(3, [VocParams(Params.Bands).FLO(which_band) VocParams(Params.Bands).FHI(which_band)]./(Params.Fs/2),'bandpass');
                % synthesis filter
                [bsy,asy] = butter(3, [VocParams(Params.Bands).FLOsy(which_band) VocParams(Params.Bands).FHIsy(which_band)]./(Params.Fs/2),'bandpass');
                
                % analysis and envelope extraction
                tmp = filtfilt(ban,aan,yraw);
                env = abs(hilbert(tmp))';
                
                %diotic carrier tone
                tmp = env.*sin(2*pi*VocParams(Params.Bands).CFsy(which_band)*traw);
                tmp = filter(bsy,asy,tmp);
                SVC = SVC + tmp;
            end
            ...
            clear which_band ban aan bsy asy tmp env
        ...
        for n_col = 1 : 5
            ...
            dt = abs(length(tt)-length(mm));
            ind1 = round(dt/2);
            if dt>0 %target word longer duration than masker word
                mm = [zeros(1,ind1),mm,zeros(1,dt-ind1)];
            else %masker word longer than target word
                tt = [zeros(1,ind1),tt,zeros(1,dt-ind1)];
            end
