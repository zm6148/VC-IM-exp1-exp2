...
for j=1:length(stimMake)
    ...
    switch stimj
        case 'marker'
            if makeGapYN
                [gapStim, refStim]=insertGap(eval(stimj),gapCenterLoc,initialGapDur,markerGapRamp,handles.Fs,stimType);
                eval([stimj '= gapStim;']);
                handles.(stimj)=[rampstim(scale2db_gap(eval(stimj),stimLevel,handles.phones,handles.Fs,refStim),stimRamp*1000,handles.Fs)];
            else
                eval([stimj '= eval(stimj);']);handles.(stimj)=[rampstim(scale2db_gap(eval(stimj),stimLevel,handles.phones,handles.Fs),stimRamp*1000,handles.Fs)];
            end
        otherwise
