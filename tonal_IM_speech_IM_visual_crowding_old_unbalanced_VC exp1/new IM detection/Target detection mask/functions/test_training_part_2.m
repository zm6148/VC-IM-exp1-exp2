...
% Last Modified by GUIDE v2.5 05-Oct-2016 13:11:57
...
pt_bw = handles.para(1,1); %pattern bandwidth 1
...
tm_dif=handles.para(1,6); %target mask loudness diff (target_loundness-mask_loudness) 6
    ...
    if sum(handles.para_test(:,7)==handles.para_test(:,13))>=8;
       ...
       close(handles.training_2);
        ...
        for j=1:TotalNumber
            ...
            m_spl_low=spl; %spline(freq,spl_c,m_low_frequences(j));
            ...
            m_spl_high=spl; %spline(freq,spl_c,m_high_frequences(j));
            phase_mask=2*pi*rand(1,1);
...
% --- Executes on key press with focus on training_2 or any of its controls.
function training_2_WindowKeyPressFcn(hObject, eventdata, handles)
% hObject    handle to training_2 (see GCBO)
