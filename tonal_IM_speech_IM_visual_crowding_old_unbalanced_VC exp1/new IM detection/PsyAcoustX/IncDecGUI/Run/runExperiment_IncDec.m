function [gapsz, trial, resps, reversalLevels,reversalBool, thresh, stdThresh, exitFlagLvl, handles]=runExperiment_IncDec(handles)
...
while reversal < nReversals    %stop run after this many reversals
    ...
    if handles.stimParams.TrackOffsetYN % tracking on SNR
        ...
        gapsz(trial,1)=handles.stimParams.initialIncDec;
        disp(['Current delta L (dB): ' num2str(gapsz(end),2)]); %display current level
        set(findall(0,'tag','depvarLevel'),'String',num2str(gapsz(end)));
        %---------------------------------------------
        
        % define maximum level ------------------------
        maxLvl=handles.stimParams.markerLevel;
    ...
    correctInt=doTrial_IncDec(handles,ISI); %present single trial of stimulus
    ...
    if answerFlag==1
        ...
        if numCorrect==2 %2 correct, change level
            if handles.stimParams.TrackOffsetYN % adjust increment / decrement
                if handles.stimParams.initialIncDec==0  % don't adjust level if increment is equal to zero.
                else
                    handles.stimParams.initialIncDec=handles.stimParams.initialIncDec-stepSize;
                    if handles.stimParams.initialIncDec<0
...
if handles.run.currentBlockNum < length(handles.blocks)
    ...
    set(findall(0,'tag','start'),'Visible','Off'); %turn on close button
end
