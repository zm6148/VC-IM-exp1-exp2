function varargout = BUG_touch_response_GUI_CRM_testing(varargin)
% BUG_TOUCH_RESPONSE_GUI_CRM_TESTING M-file for BUG_touch_response_GUI_CRM_testing.fig
%      BUG_TOUCH_RESPONSE_GUI_CRM_TESTING, by itself, creates a new BUG_TOUCH_RESPONSE_GUI_CRM_TESTING or raises the existing
%      singleton*.
%
%      H = BUG_TOUCH_RESPONSE_GUI_CRM_TESTING returns the handle to a new BUG_TOUCH_RESPONSE_GUI_CRM_TESTING or the handle to
%      the existing singleton*.
%
%      BUG_TOUCH_RESPONSE_GUI_CRM_TESTING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BUG_TOUCH_RESPONSE_GUI_CRM_TESTING.M with the given input arguments.
%
%      BUG_TOUCH_RESPONSE_GUI_CRM_TESTING('Property','Value',...) creates a new BUG_TOUCH_RESPONSE_GUI_CRM_TESTING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before BUG_touch_response_GUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to BUG_touch_response_GUI_CRM_testing_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help BUG_touch_response_GUI_CRM_testing

% Last Modified by GUIDE v2.5 24-Aug-2018 16:03:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @BUG_touch_response_GUI_CRM_testing_OpeningFcn, ...
                   'gui_OutputFcn',  @BUG_touch_response_GUI_CRM_testing_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before BUG_touch_response_GUI_CRM_testing is made visible.
function BUG_touch_response_GUI_CRM_testing_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to BUG_touch_response_GUI_CRM_testing (see VARARGIN)

% Choose default command line output for BUG_touch_response_GUI_CRM_testing
...
if strcmp(handles.display_mode,'light')
    ...
    set(children_list(button_children_index),'BackgroundColor',[0.8 0.8 0.8]);
...
[a,map]=imread('blue.jpg');
I=imresize(a, [60 140]);
set(handles.B23,'CData',I);

[a,map]=imread('red.jpg');
I=imresize(a, [60 140]);
set(handles.B25,'CData',I);

[a,map]=imread('green.jpg');
I=imresize(a, [60 140]);
set(handles.B24,'CData',I);

[a,map]=imread('white.jpg');
I=imresize(a, [60 140]);
set(handles.B26,'CData',I);

% Update handles structure
guidata(hObject, handles);

feedback_children_index = 2:3;
button_children_index = [8:47]-3;
not_used_button_children_index = [[15:21],45,[8:15],44,32,33,31,[36:43],47]-3;
% button_children_index_not_used = 8;
switch handles.op_flag
    ...
    case 'trial'
        ...
        set(children_list(not_used_button_children_index),'Enable','off')
        set(children_list(not_used_button_children_index),'Visible','off')
    ...
    case 'feedback'
        ...
        for n_word = 2:3
            ...
            eval(['set(handles.t',num2str(n_word),',''String'',feedback.target{word_index})'])
            if strcmp(feedback.target{word_index},feedback.response{word_index})
...
function varargout = BUG_touch_response_GUI_CRM_testing_OutputFcn(hObject, eventdata, handles) 
...
if strcmp(handles.order_flag,'on');
    ...
    if length(find(button_col_check(1:n_col-1)>0))==n_col-1
        ...
        handles.button_list(n_col,color_index)=max(max(handles.button_list))+1; %store response
        if max(max(handles.button_list))==2
            uiresume(handles.figure1);
        end
