function   write_results(handles,results_pathname)
...
fseek(fid,0,'eof');
fprintf (fid, '-------------------------------------------\n');
fprintf (fid, 'Level (dB):  Resp.(1=corr.; 0=incorr.):\n');
fprintf (fid, '-------------------------------------------\n');
fprintf(fid,sprintf('%d     %d%s\n', data{:}));
fprintf (fid, '------------------------------------------------------------\n');
fprintf (fid, 'Reversal Levels (dB): ');
fprintf(fid,'%d ', reversalLevels);
fprintf(fid,'\n');
fprintf (fid, 'Threshold (dB): %.2f\n', thresh);
fprintf (fid, 'SD (dB): %.2f\n', stdThresh);
fprintf (fid, '------------------------------------------------------------\n');
fprintf (fid, '\n');
fclose (fid);
