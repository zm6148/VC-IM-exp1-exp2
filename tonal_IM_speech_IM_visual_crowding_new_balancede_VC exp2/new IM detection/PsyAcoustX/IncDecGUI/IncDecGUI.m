function varargout = IncDecGUI(varargin)
% INCDECGUI M-file for IncDecGUI.fig
%
...
% Last Modified by GUIDE v2.5 04-Jun-2014 09:42:33

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @IncDecGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @IncDecGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before IncDecGUI is made visible.
function IncDecGUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to IncDecGUI (see VARARGIN)

% Choose default command line output for IncDecGUI
...
handles.stimParams.ramp=0.005;
% handles.stimParams.markerGapRamp=0.001;
handles.stimParams.markerLNNToggle=0;
handles.stimParams.markerType='Tone';
handles.stimParams.markerCutoffs= [1000];

% Gap defaults
handles.stimParams.gapCenterLoc=0.5; % default is center of the pedestal (50%)
handles.stimParams.initialGapDur=0.01; % default increment / decrement duration
handles.stimParams.TrackOffsetYN=1; % if set 1 one, the task is decrement / increment detection in zero, gap detection
handles.stimParams.DecrementYN=1;
handles.stimParams.initialIncDec=6;

% % increment / decrement signal defaults
% if handles.stimParams.gapCenterLoc==0.485
%     handles.stimParams.initialIncDec=10;
% elseif handles.stimParams.gapCenterLoc==0.02
%     handles.stimParams.initialIncDec=15;
% else
%     handles.stimParams.initialIncDec=10;
% end

% Background noise defaults
handles.stimParams.bnoiseToggle=0;      %present background noise to restrict off-frequency listening?
...
handles.stimParams.bnoiseOffset=-40;     %offset (in dB) between marker and background noise
...
handles.earPres='L';              %mode of presenation (RE alone = 'R'; LE alone = 'L'; binaural  = 'B')
handles.run.nReversals=10;        % # of reversals to track
handles.run.nReversalConsider=6;  % # of reversals to consider for average

% handles.stimParams.noiseBW=[80 20000];  %masker cutoff frequencies for broadband masker

handles.run.ISI=0.400;            %sec
handles.run.stepSize.initial=5;   %initial Inc/Dec size
handles.run.stepSize.final=1; % final Inc/Dec size
...
% UIWAIT makes IncDecGUI wait for user response (see UIRESUME)
% uiwait(handles.gapGUI_FIG);


% --- Outputs from this function are returned to the command line.
function varargout = IncDecGUI_OutputFcn(hObject, eventdata, handles) 
...
[action, handles.stimParams, handles.blocks] = selectStimParams_IncDec(handles.stimParams,handles.blocks); %brings up stimulus select window

switch(action)
    case 'continue'  
        
        [handles, buffer]=makeStim_IncDec(handles,1); %creates stimulus from user defined parameters.  Second argument tells program to make the example with a gap (as opposed to without the gap)        
