function [Fs,sound] = build_sound_tonal_mask_mono_linear_test( handles,delay_t )
...
Delay = delay_t; %delay may be random mark for edit later
...
%         end
        ...
        f_low = linspace(m_low_cf - 300, m_low_cf, TotalNumber);
        f_high = linspace(m_high_cf, m_high_cf + 300,TotalNumber);
...
for j=1:TotalNumber
    ...
    switch t_pt
        ...
        case 1 % target there
            ...
            tone=(tone/rms(tone))*10^((tm_dif-101.3)/20);
            target_pattern = [target_pattern, tone, silence];
    end
    
    m_spl_low = 40 + 10*(log10(300)); %spline(freq,spl_c,m_low_frequences(j));
    phase_mask=2*pi*rand(1,1);
    tone = envelope.*sin(2*pi*m_low_frequences(j)*t+phase_mask);
    tone=(tone/rms(tone))*10^((m_spl_low-101.3)/20);
    mask_low_pattern = [mask_low_pattern, tone, silence];
    
    m_spl_high = 40 + 10*(log10(300)); %spline(freq,spl_c,m_high_frequences(j));
    phase_mask=2*pi*rand(1,1);
    tone = envelope.*sin(2*pi*m_high_frequences(j)*t+phase_mask);
    tone=(tone/rms(tone))*10^((m_spl_high-101.3)/20);
