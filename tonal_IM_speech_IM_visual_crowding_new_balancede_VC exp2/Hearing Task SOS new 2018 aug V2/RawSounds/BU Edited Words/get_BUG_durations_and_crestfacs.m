...
BUG_word_durations_secs = zeros(ntalkers,nwordrows,nwordcols);
...
for nt = 1:ntalkers
    ...
    for nrows = 1:nwordrows
        for ncols = 1:nwordcols
            ...
            rms = sqrt(mean(w.^2));
            BUG_word_crestfactors(nt,nrows,ncols) = max(abs(w))/rms;
        end;
    end;
end;

save BUG_word_durations_and_crestfacs BUG_word_durations_secs BUG_word_crestfactors;
